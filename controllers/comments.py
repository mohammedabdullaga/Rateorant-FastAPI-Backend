...
from serializers.comment import CommentSchema, CommentCreateSchema, CommentUpdateSchema
...
def create_comment(tea_id: int, comment: CommentCreateSchema, db: Session = Depends(get_db)):
    tea = db.query(TeaModel).filter(TeaModel.id == tea_id).first()
    if not tea:
        raise HTTPException(status_code=404, detail="Tea not found")
...
def update_comment(comment_id: int, comment: CommentUpdateSchema, db: Session = Depends(get_db)):
